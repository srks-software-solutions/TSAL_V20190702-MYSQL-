...
namespace Tata.Controllers
{
    public class ReportController : Controller
    {
            ...
            do
            {
                ...
                for (int n = 0; n < machin.Rows.Count; n++)
                {
                    ...
                    else
                    {
                        ...
                        DTConsolidatedLosses.Rows.Add(HierarchyData[0], HierarchyData[1], HierarchyData[2], HierarchyData[3], HierarchyData[4], dateforMachine, ValueAddingTime, AvailableTime, SummationOfSCTvsPP, ScrapQtyTime, ReWOTime, ValueAddingTime, IsWorking, AvailableTime, 24, 0, minorLossTime, BreakdownTime, blueTime);
                    }
                    //Now get & put Losses
        ...
        public void UtilizationReportExcel(string StartDate, string EndDate, string PlantID, string ShopID = null, string CellID = null, string WorkCenterID = null, string TabularType = null)
        {
            ...
            do
            {
                ...
                for (int n = 0; n < machin.Rows.Count; n++)
                {
                    ...
                    double ROAMainLosses = Math.Round(((ROALossess - SettingTime) / 60), 1);
                    ...
                    worksheet.Cells["J" + Row].Value = ROAMainLosses;
            ...
            for (int i = 0; i < DTUtil.Rows.Count; i++)
            {
                ...
                worksheet.Cells["H" + Row].Value = Math.Round(valueAddingSummation1 / (60), 2);
                worksheet.Cells["I" + Row].Value = Math.Round(setTimeSummation1 / (60), 2);
                worksheet.Cells["J" + Row].Value = Math.Round(idleTimeSummation1, 2);
                worksheet.Cells["K" + Row].Value = Math.Round(minorLossTimeSummation1 / (60), 2);
                worksheet.Cells["L" + Row].Value = Math.Round(BreakdownTimeSummation1 / (60), 2);
                worksheet.Cells["M" + Row].Value = Math.Round(blueTimeSummation1 / (60), 2);
                worksheet.Cells["N" + Row].Value = Math.Round(TotalTimeSummation1 / (60), 2);
        ...
        //    FileInfo templateFile = new FileInfo(@"C:\TataReport\NewTemplates\JobReportNew.xlsx");
        //    ExcelPackage templatep = new ExcelPackage(templateFile);
        //    ExcelWorksheet Templatews = templatep.Workbook.Worksheets[1];
        //    ExcelWorksheet TemplateGraph = templatep.Workbook.Worksheets[2];
        ...
        //                        var MeasuringUnitId = db.tblpartwiseworkcenters.Where(m => m.WorkCenterId == MachineIDRow).FirstOrDefault();
        ...
        //                                    }
        //                                    else if (StartRows != 0 && (StartRows != 0 || ji == HMIDATA.Rows.Count))
        //                                    {
        //                                        EndRows = Row - 1;
        //                                        BatchGreen += Convert.ToDouble(HMIDATA.Rows[ji - 1][13]);
        //                                    }
                                ...
                                else
                                {
                                    ...
                                    else if (Convert.ToInt32(HMIDATA.Rows[ji][28]) == 1) //Its a ManualWC
                                    {
                                        ...
                                        if (MeasuringUnitId != null)
                                        {
                                            ...
                                            if (ji != 0)
                                            {
                                                if (!string.IsNullOrEmpty(HMIDATA.Rows[ji - 1][36].ToString()))
                                                {
                            ...
                            if (Convert.ToInt32(row.IsMultiWO) == 1) //Its a MultiWorkOrder
                            {
                                ...
                                foreach (var MulitWORow in MulitWOData)
                                {
                                    if (Convert.ToDouble(MulitWORow.NCCuttingTimePerPart) == 0)
                                    {
                                        ...
                                        worksheet.Cells["V" + Row].Formula = "=SUM(Q" + Row + ",R" + Row + ",S" + Row + ",W" + Row + ")";
                                        double BreakdownDuration = Convert.ToDouble(row.Breakdown);
                                        worksheet.Cells["W" + Row].Value = Math.Round(BreakdownDuration, 1);
                                        worksheet.Cells["X" + Row].Value = 0;
                                        worksheet.Cells["Y" + Row].Value = "0";
                                        worksheet.Cells["Z" + Row].Value = row.OperatorName;
                                        worksheet.Cells["AA" + Row].Value = row.Type;
                                        worksheet.Cells["AB" + Row].Value = row.PartNo + "#" + row.OpNo;
                                        double stdCuttingTime = 0;
                                        double ProdOfstdCuttingTimeDelivQty = 0;
